 ...
 * - 폴더를 만들어서 파일을 저장하고 관리하듯이 패키지를 만들어 클래스를 저장하고 관리한다
   ...
   - 모든 클래슨느 하나의 패키지에 속하며, 패키지가 선언되지 않는 클래스는 자동적으로 이름 없는 패키지(default)에 속하게 된다.
 */
package com.kh;
/*
 * 클래스(class) ↓
 * - 자바에서 모든 코드는 반드시 클래스안에 존재해야 하며 서로 관련된 코드들을 그룹으로 나누어 별도의 클래스를 구성한다.
 * - 클래스들이 모여 하나의 Java 프로그램으로 구성한다.
 * */
public class Hello { //클래스 명은 클래스 파일명과 동일해야한다. !!!!!!//

	public static void main(String[] args) { // 실행 메소드(main 메소드) : 자바 프로그램 실행 시 java.exe에 의해 가장 먼저 호출한다.//
 ...
 * */
	 ...
	 * - 메소드명 : 소문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마다 대분자로 표기 ex) helloWorld
	 * 
	 * 
	 * 
	 *  자바의 특징
